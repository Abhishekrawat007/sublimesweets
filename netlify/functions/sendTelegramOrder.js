...
export async function handler(event) {
  ...
  return `• ${title} (x${qty}) - Rs. ${price * qty}${customMsg}`;
    ...
    pdf.setFillColor(250, 245, 255); // Royal Purple & Gold (Luxury Jewelry) background
    pdf.rect(0, 0, pageWidth, pageHeight, 'F');

    // PREMIUM ORANGE HEADER BAR (single solid color)
    pdf.setFillColor(102, 45, 145); // Royal Purple & Gold (Luxury Jewelry) header
    pdf.rect(0, 0, pageWidth, 90, 'F');

    // Gold decorative line below header
    pdf.setDrawColor(255, 215, 0); // Royal Purple & Gold (Luxury Jewelry) accent
    pdf.setLineWidth(2);
...
rows.push([
  ...
  `Rs. ${item.price * item.qty}`
    ...
    rows.push([
      ...
      { content: `Rs. ${computedTotal}`, styles: { halign: "right", fontStyle: "bold", fontSize: 14, textColor: [102, 45, 145] } }
    ]);
    ...
    autoTable(pdf, {
      ...
      headStyles: { 
        fillColor: [102, 45, 145], // Royal Purple & Gold (Luxury Jewelry) table header
      ...
      alternateRowStyles: {
        fillColor: [255, 248, 240] // Royal Purple & Gold (Luxury Jewelry) alternate rows
      },
      columnStyles: { 
        0: { cellWidth: 35 }, 
        1: { cellWidth: 50 }, 
    ...
    pdf.setFontSize(11);
    pdf.setTextColor(75, 0, 130); // Royal Purple & Gold (Luxury Jewelry) footer
    pdf.text(
