...
export async function handler(event) {
  ...
  await admin.database().ref('sites/sabsepyarastore/pushSubscribers/' + token).remove();
  console.log('Removed invalid token:', token.slice(0,16));
      ...
      try {
        ...
        await admin.database().ref('sites/sabsepyarastore/pushSubscribers/' + token + '/topics/' + topic).set(meta);
      } catch (e) {
      ...
      try {
        await admin.database().ref('sites/sabsepyarastore/pushSubscribers/' + token + '/topics/' + topic).set({
          subscribed: false,
