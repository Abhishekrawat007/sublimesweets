...
async function removeInvalidTokens(tokensToRemove = []) {
  ...
  for (const t of tokensToRemove) {
    try {
      await admin.database().ref('sites/sabsepyarastore/pushSubscribers/' + t).remove();
      const shortKey = t.slice(0, 24).replace(/[^a-zA-Z0-9_-]/g, '');
      await admin.database().ref('sites/sabsepyarastore/tokens/' + shortKey).remove().catch(()=>{});
...
export async function handler(event) {
  ...
  try {
    if (event.httpMethod === "GET" && event.queryStringParameters?.testToken === "1") {
      const snap = await admin.database().ref("sites/sabsepyarastore/pushSubscribers").limitToFirst(1).once("value");
    ...
    const dbRef = admin.database().ref("sites/sabsepyarastore/pushSubscribers");
